...
class FeedbackBasedPredictionModel:
    ...
    def clear_data(self, reset_type: str = 'all'):
        """
        Clear data from the prediction model to reset it
        ...
        if reset_type == 'all' or reset_type == 'weights':
            # Reset prediction weights to defaults
            ...
            if HAS_STATSMODELS:
                self.prediction_weights["time_series"] = 0.15
                
            if HAS_PYTORCH:
                self.prediction_weights["deep_learning"] = 0.2
                
            # Normalize weights
            self._normalize_prediction_weights()
        
        if reset_type == 'all' or reset_type == 'scalers':
            # Reset all scalers
            self.weights_scaler = None
            self.feature_scaler = None
            self.exercise_encoder = None
            
            # Reset advanced models
            self.arima_models = {}
            ...
            try:
                if os.path.exists(self.weights_scaler_path):
                    os.remove(self.weights_scaler_path)
                if os.path.exists(self.feature_scaler_path):
                    os.remove(self.feature_scaler_path)
                if os.path.exists(self.exercise_encoder_path):
                    os.remove(self.exercise_encoder_path)
                if os.path.exists(self.arima_models_path):
                    os.remove(self.arima_models_path)
                if os.path.exists(self.lstm_model_path):
                    os.remove(self.lstm_model_path)
        ...
        return {
            "success": True,
            "message": f"Successfully cleared {reset_type} data from the prediction model"
        }
    ...
    def _compute_engineered_features(self, previous_workouts: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        # Process RIR data if available
        rir_values = [w.get('rir', None) for w in previous_workouts]
        valid_rir = [r for r in rir_values if r is not None]
        
        if valid_rir:
            features['avg_rir'] = np.mean(valid_rir)
            # Calculate if recent workouts have been too easy (high RIR)
            if len(valid_rir) >= 3:
                recent_rir = valid_rir[-3:]
                features['recent_avg_rir'] = np.mean(recent_rir)
                
                # Flag for whether the weights should be increased based on RIR
                if features['recent_avg_rir'] >= 3:
                    features['rir_increase_flag'] = 1.0
                elif features['recent_avg_rir'] >= 2:
                    features['rir_increase_flag'] = 0.5
                elif features['recent_avg_rir'] <= 0.5:
                    # Very low RIR means exercise was very challenging
                    features['rir_increase_flag'] = -0.3
                else:
                    features['rir_increase_flag'] = 0.0
    ...
    def _calculate_one_rep_max(self, weight: float, reps: int, formula: str = 'brzycki') -> float:
        ...
        if formula.lower() == 'epley':
            ...
            # Mayhew formula: 1RM = w / (0.522 + 0.419 * e^(-0.055 * r))
            import math
    ...
    def __init__(self, model_dir: str = None):
        ...
        self.prediction_weights = {
            "last_weight": 1.05,  # Weight given to the last workout's weight
            "avg_progress": 0.2,  # Weight given to the average progress rate
            "consistency": 0.1,   # Weight given to consistency factor
            "volume": 0.1,        # Weight given to total volume factor
        }
    ...
    def predict(self, exercise: str, previous_workouts: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if engineered_features:
            ...
            # Apply RIR-based adjustments if available
            if 'rir_increase_flag' in engineered_features:
                rir_flag = engineered_features['rir_increase_flag']
                # Adjust prediction based on RIR flag
                # Positive flag means workouts have been too easy (high RIR), so increase weight
                # Negative flag means workouts have been too hard (low RIR), so decrease weight
                rir_adjustment = rir_flag * 0.05  # 5% adjustment per unit of flag
                weighted_prediction = weighted_prediction * (1 + rir_adjustment)
        ...
        if len(previous_workouts) >= 5:
            ...
            # Add RIR analysis if available
            if 'recent_avg_rir' in engineered_features:
                rir_avg = engineered_features['recent_avg_rir']
                if rir_avg >= 3:
                    analysis["difficulty"] = "too easy"
                    response["message"] = f"Weight increased due to high RIR (average {rir_avg:.1f})"
                elif rir_avg >= 2:
                    analysis["difficulty"] = "somewhat easy"
                elif rir_avg <= 0.5:
                    analysis["difficulty"] = "very challenging"
                    if rir_avg == 0:
                        response["message"] = "Weight adjusted based on maximum effort (0 RIR)"
                else:
    ...
    def provide_feedback(self, 
        ...
        # Calculate base score: positive if prediction was too low, negative if too high
        # Scale is from -1 to 1 where 0 is perfect
        score = max(min(relative_diff, 1.0), -1.0)
        
        # Apply RIR adjustment to the score if available
        if rir is not None:
            # RIR adjustment: 
            # - High RIR (3+) means the weight was too light, so increase the score (suggesting higher weight)
            # - Low RIR (0-1) means the weight was challenging, potentially appropriate
            if rir >= 3:
                # Weight was too light, increase positive adjustment
                rir_adjustment = min(rir * 0.05, 0.3)  # Cap at 0.3 (30% adjustment)
                if score >= 0:
                    # Already positive score, enhance it
                    score = min(score + rir_adjustment, 1.0)
                else:
                    # Negative score (predicted too high), but actually too light, reduce negative factor
                    score = min(score + rir_adjustment * 2, 0.5) # Allow it to flip to positive
            elif rir == 0:
                # Weight was very challenging (no reps in reserve)
                # If the prediction was already too high (negative score), enhance that
                if score < 0:
                    score = max(score - 0.1, -1.0)  # Make it more negative
    ...
    def _generate_intensity_based_reps(self, previous_weight: float, previous_reps: int, predicted_weight: float) -> List[int]:
        ...
        # FIXED: The rep scheme logic needs to respect the target reps
        # If we're predicting for a specific rep target, use that instead of the intensity-based suggestion
        if previous_reps > 0:
            # Use the previous rep count as the target
            target_reps = previous_reps
        else:
            # Default to the intensity-based suggestion
            target_reps = suggested_base_reps
            
        # Generate a rep scheme based on the target reps
        if target_reps <= 5:
            # For low rep ranges (1-5), keep rep count consistent across sets
            # Most strength programs have consistent reps for heavy weights
            return [target_reps, target_reps, target_reps]
        elif target_reps <= 8:
            # For moderate rep ranges (6-8), slight drop-off is common
            return [target_reps, target_reps - 1, target_reps - 1]
        else:
            # For higher rep ranges (9+), larger drop-off is common due to fatigue
            return [target_reps, target_reps - 2, target_reps - 2]
        """
        Generate rep scheme based on intensity relationship 
    ...
    def _calculate_data_dependent_scaling(self, previous_workouts: List[Dict[str, Any]]) -> float:
        """
        Calculate a scaling factor that adjusts prediction responsiveness based on data quantity
        
        When there's little data, we want to be more responsive to feedback
        As more data accumulates, we become more conservative
        
        Args:
            previous_workouts: List of previous workout data
            
        Returns:
            Scaling factor (higher = more responsive to feedback)
        """
        workout_count = len(previous_workouts)
        
        # For very few workouts, be very responsive
        if workout_count < 3:
            return 3.0
        # For a small amount of data (less than 10 workouts), scale down gradually
        elif workout_count < 10:
            # Scale from 3.0 at 3 workouts to 1.0 at 10 workouts
            return 3.0 - (2.0 * (workout_count - 3) / 7)
        # Normal responsiveness for 10-20 workouts
        elif workout_count < 20:
            return 1.0
        # Reduced responsiveness for larger datasets
        else:
            # Scale down to 0.5 for 30+ workouts
            return max(0.5, 1.0 - (0.5 * min(workout_count - 20, 10) / 10))
